...
async function scrapeAllCategories() {
  ...
  if (isSupabaseEnabled) {
    ...
    const formattedProducts = allProducts.map(p => ({
      name: p.name,
      price: p.price,
      url: p.imageUrl || `https://shwapno.com/product/${encodeURIComponent(p.name.toLowerCase().replace(/\s+/g, '-'))}`
    }));
    await saveToSupabase(storeId, 'Shwapno', formattedProducts, ourProductsMap);
  }
