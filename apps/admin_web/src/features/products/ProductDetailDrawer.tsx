...
export function ProductDetailDrawer({ productId, onClose, onEdit }: ProductDetailDrawerProps) {
  ...
  const deactivateMutation = useMutation({
    ...
    onError: (err: unknown) => notify((err as Error).message || 'Failed to deactivate product', 'error'),
  });
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="card p-4">
              ...
              <div className="text-xl font-bold">{(product as any).current_qty || 0}</div>
            </div>
          </div>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody>
                  {isHistoryLoading ? (
                    ...
                    stockHistory.map((log: { id: string; created_at: string; delta: number; reason: string }) => (
