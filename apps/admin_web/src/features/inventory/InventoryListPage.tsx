...
export function InventoryListPage() {
  ...
  const stats = useMemo(() => {
    const all = (inventory as any[]) ?? [];
    const total = all.length;
    const lowStock = all.filter((p: any) => p.reorder_status === 'LOW').length;
    const outOfStock = all.filter((p: any) => p.reorder_status === 'OUT').length;
    const totalValue = all.reduce((sum: number, p: any) => sum + ((p.price || 0) * p.current_qty), 0);
