  ...
  return (
    <Link
      ...
      className={`flex flex-col items-center justify-center gap-1 min-h-[44px] px-5 relative transition-all duration-200 ${
        isActive ? 'text-[#ffe721]' : 'text-gray-600 hover:text-gray-800'
      }`}
      aria-current={isActive ? 'page' : undefined}
    >
      {isActive && (
        <span className="absolute -top-0.5 left-1/2 -translate-x-1/2 w-5 h-[3px] bg-[#ffe721] rounded-full" />
      )}
      <span className="text-[22px] leading-none" aria-hidden="true">{icon}</span>
      <span className="text-[10px] font-semibold">{label}</span>
      {badgeCount > 0 && (
        <span className="absolute top-1 right-3 min-w-[16px] h-4 bg-[#ffe721] text-[#1c1917] text-[10px] font-bold rounded-full grid place-items-center px-1">
          {badgeCount}
        </span>
      )}
    </Link>
