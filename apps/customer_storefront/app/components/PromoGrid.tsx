...
export function PromoGrid({ promos = defaultPromos }: { promos?: PromoItem[] }) {
  ...
  return (
    <section className="mb-8" aria-label="Promotions">
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 overflow-x-auto md:overflow-visible snap-x snap-mandatory scrollbar-hide scroll-edge-mask px-4 sm:px-6 lg:px-8 xl:px-10 pb-1">
        <Link
        ...
        >
          ...
          <div className="relative z-20 w-full md:w-4/5">
            ...
            <span className="inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1 text-xs font-bold text-black hover:bg-gray-100 transition-colors press-feedback">
              {largePromo.ctaText}
