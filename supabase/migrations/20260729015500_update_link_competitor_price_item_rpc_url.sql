-- Migration: 20260729015500_update_link_competitor_price_item_rpc_url.sql
-- Description: Update link_competitor_price_item RPC to accept optional custom competitor URL mapping.

DROP FUNCTION IF EXISTS public.link_competitor_price_item(uuid, uuid, uuid);
DROP FUNCTION IF EXISTS public.link_competitor_price_item(uuid, uuid, uuid, text);
...
BEGIN
  ...
  SELECT competitor_price, competitor_product_url INTO v_comp_price, v_url
  ...
  SET
    ...
    competitor_product_url = COALESCE(NULLIF(trim(p_competitor_product_url), ''), v_url),
